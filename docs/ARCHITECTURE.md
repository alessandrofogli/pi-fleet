...
### `bin/herdr-launch.sh` — native initial-request delivery (gh-7, heir of T-027/T-029)
The CHILD_PROMPT (rules + delivery posture + brief) arrives as pi's **native initial request**: the launcher materializes it in a **private transient file** `$STATE_HOME/<task-id>.child-prompt.md` and `agent start ... -- [--model provider/id] @<prompt-file>` hands pi the `@<file>` argv element — pi reads the file and prompts the session BEFORE its main loop (`session.prompt(initialMessage)` in `dist/modes/interactive/interactive-mode.js`; contract verified against the installed pi 0.84.x with a read-only `parseArgs` fixture in the smoke). **ZERO `agent prompt` calls for the brief**, and there is NO post-start readiness/`interactive_ready` wait, NO ACK/consumption retry, NO session-file snapshot, NO fallback delivery: the initial request is born inside the pi process at `agent start`, so **delivery IS `agent start` returning OK**. A frozen/empty pane is still caught by the §7 done-wait liveness gate (`agent_alive`), exactly as before.
- **Herdr argv contract (verified against installed herdr 0.8.2 + its skill)**: `herdr agent start <NAME> --kind <KIND> --pane <ID> -- [AGENT_ARG]...` forwards native agent arguments raw after `--` — one complete argv element per arg, array-safe (spaces/metacharacters stay inside the element). The launcher passes EXACTLY ONE `@`-element `"@$CHILD_PROMPT_PATH"`, kept separate from the `--model provider/id` pair (always the full qualified id, never a bare id).
- **Why a file, not a positional message**: herdr refuses args it cannot shell-encode safely (multi-line text with quotes/`$`/backticks → `invalid_agent_argument`); the `@<file>` indirection carries the prompt body off-argv. Only the path crosses herdr argv/debug output, never the prompt contents.
- **Transient prompt-file lifecycle**: written before `agent start` with `umask 077` and an atomic unique tmp sibling + `mv`; validated (absolute, regular, non-empty, readable) BEFORE any herdr call — a missing/unreadable prompt file fails the task (state `failed`), never sends an empty request. Removed by an EXIT trap on EVERY terminal path (success, failure, abort, signal, timeout, relaunch handoff); a per-task-id stale sweep before the write reclaims leftovers of a SIGKILLed previous run. Durable brief files (`tasks/<id>.brief.md`) are NEVER removed by this cleanup.
- **Resume (`--resume`, T-019)**: rebuilds the transient prompt file from the SAME CHILD_PROMPT + RESUME NOTICE (never reuses a deleted file, never a second manual prompt) and goes through the same native delivery — one code path, no second delivery mechanism.
Headless smoke: `tests/smoke-prompt-ack.sh` (mocked herdr/treehouse per test, isolated temp state): the full issue gh-7 10-point plan — exactly one `@` argv element, absolute, existing at invocation, bytes == the complete expected CHILD_PROMPT; model as a separate `--model provider/id` pair; state path + brief with spaces and shell metacharacters; ABSENCE of `agent prompt`/`interactive_ready` wait/ACK retry/`brief delivered` log; done-marker completion + tab/pane/worktree cleanup; start-failure and unreadable prompt-file → failed state + cleanup (never stranded in spawning); 3 concurrent tasks with distinct IDs/panes/agent names and zero prompt cross-contamination; `--resume` rebuild; installed-Pi `parseArgs` fixture (read-only).
...
├── captain-claim.<slug>.json      # issue #13 single-captain claim/heartbeat (per project, opt-in: only
│                                   #   written by PI_FLEET_CAPTAIN=1 sessions) {project, sessionId, host, claimedAt, lastBeatAt}
├── <id>.log                        # launcher log
...
```json
{
  ...
  "captainId": "<PI_SESSION_ID of the launching captain>"   // issue #13: which captain launched the task
}
```
...
| **Audible reply** (`notify.sound`) | hook `agent_settled` in `extensions/index.ts` + `extensions/fleet-sound.ts` | Terminal bell (BEL) when the captain's user-visible turn settles — direct replies and wake-and-report digests; **never from children** (IS_CAPTAIN gate) and **never from the silent `display:false` wakes** (role `custom`, not counted as assistant output). Config: captain preference `notify.sound: on\|off` (default `on`); barrier group = one digest = one bell, `groupMode: streaming` = one bell per task reply |
| **Owner-safe worktree release** (gh-8) | `bin/fleet-cleanup.sh` + `bin/herdr-launch.sh` + `extensions/index.ts` (`reconcileStaleTasks`, `fleet_abort`) | **Outcome and cleanup are separate state machines.** At acquisition the launcher persists the lease identity with the task record (`worktreePath`, `leaseHolder` `pi-fleet:<task-id>`, `leaseId` from `treehouse get --json`, `leaseAcquiredAt`). ONE shared idempotent cleanup owner (`bin/fleet-cleanup.sh`) is used by every path (normal completion, abort, failure, timeout, crash, reconciliation): it re-reads `treehouse status --json`, returns ONLY with the exact guard (`--if-lease-id` preferred, `--if-lease-holder` fallback), never suppresses the real exit status, and persists `released \| already_released \| conflict \| pending \| failed` in `<id>.cleanup.json` (mirrored into the state `.cleanup`) with a per-task lock + stale takeover. Active tasks are NEVER auto-released (an interrupted launcher marks the task failed first). The reconciler classifies terminal tasks report-only by default (`FLEET_RECONCILE_RELEASE=1` opts into guarded release); `fleet_abort` owns the cleanup and reports the durable result, never a claim. Before the transient done-marker is consumed, the artifact bundle is persisted outside the worktree: `<id>.result.json` (outcome + FULL original done-marker payload + report metadata), `<id>.findings.json` (validated BLOCKING/NON_BLOCKING), `<id>.report.md` (scout report copy with size/sha256 — `reportPath` alone is NOT durable). Tests: `tests/smoke-gh8-cleanup.sh` (owner + launcher, fake treehouse/herdr, 80 checks) and `tests/smoke-gh8-extension.sh` (compiled-extension driver, 18 checks). Review skill: `skills/fleet-ts-review/SKILL.md`. |
| **Calm mode** (`calm` pref, gh-9) | `extensions/index.ts` activation hook + `extensions/lib/fleet-calm*.ts` | Firstmate-style quiet transcript while running: hides pi-fleet's operational rows — collapsed thinking, mid-turn assistant working notes, pi-fleet user-role wake/health envelopes (`FLEET WATCHER WAKE:` / `T-019 health watchdog (`), the visible `fleet_notice` custom rows, the wrapped built-in tool shells (read/bash/edit/write/grep/find/ls via a `renderShell: "self"` rebuild of pi's Box shell using only the render-slot theme) — and swaps the stock Working… row for a static quiet line (`setWidget` string-overload + `setWorkingVisible(false)`, boat animation consciously **not** ported: timer-free/deterministic/headless-testable). Toggle: `/fleet-calm` command (kebab-case, avoiding `calm` ambiguity) or captain preference `calm: on\|off` (default off) via the canonical `fleet-learn` getPref/setPref. Presentation-only: never mutates message data/context/persistence. `@earendil-works/pi-tui` resolved lazily: tool-shell hiding uses structural empty components, the stock-shell reconstruction only applies when pi-tui resolves (missing → diagnostic + graceful degradation). Seams probe at install and degrade with a diagnostic if missing (missing-seam, partial-UI-context and duplicate-toggle guarded). Tests: `tests/smoke-calm.sh` (A tsc, B emit fixture, C 56-check main driver on real pi components, D seam-degradation fresh process, E existing smoke-prompt-ack.sh + smoke-sound.sh stay green). |
| **Single-captain / wave-ownership guard** (issue #13) | `bin/fleet-captain-lib.sh` + `bin/herdr-launch.sh` guard hook | Prevents the dual-captain overlap (local tmux + remote pi-bridge on the same host). **Ownership (opt-in)**: only `PI_FLEET_CAPTAIN=1` sessions write/check a per-project claim `captain-claim.<slug>.json` ({project, sessionId=PI_SESSION_ID, host, claimedAt, lastBeatAt}); the acting session must own the claim (refreshed as a heartbeat on every launch) or be refused — a stale claim (lastBeatAt > FLEET_CAPTAIN_STALE_S, default 900s) is stolen. **Duplicate live group (always on, refuses only CLEAR duplicates)**: before a labeled launch, a LIVE same-label DIFFERENT-groupId group for the same project is refused; same-wave members and terminal waves never block. A refused launch writes a FAILED task record and exits 1 BEFORE the workspace/worktree sections (no lease/pane). Laptops w/o pi-bridge (PI_FLEET_CAPTAIN unset, no claim) are fail-open: behavior unchanged. `captainId` is logged + persisted in the task state. Tests: `tests/smoke-captain-guard.sh` (22 checks, scratch state). |
