...
### `bin/herdr-launch.sh` — native initial-request delivery (T-029)
The CHILD_PROMPT (rules + delivery posture + brief) arrives as pi's **native initial request**: the launcher materializes it at `~/.pi/fleet/tasks/<id>.prompt.md` and `agent start ... -- <model args> @<prompt-file>` hands pi the `@<file>` arg — pi's interactive mode reads the file and prompts the session BEFORE its main loop (`initialMessages` → `session.prompt(...)`, verified in the installed pi `dist/modes/interactive/interactive-mode.js`). **ZERO `agent prompt` calls for the brief**: there is no delivery race (the prompt is born inside the child process — it cannot be buffered/lost), no readiness waver, no consumption-ACK retry.
- **Why not a plain positional message**: herdr forwards agent args raw into pi's argv but REFUSES anything it cannot shell-encode safely (multi-line text with quotes/`$`/backticks → `invalid_agent_argument`, verified empirically) — hence the `@<file>` indirection (`<id>.prompt.md` under `~/.pi/fleet/tasks/`).
- **Delivery point**: `agent start` returning OK IS delivery — herdr confirms the agent was detected and ready (`interactive_ready`); the launcher logs `brief delivered to the child (native initial request)` immediately after.
- **Residual check (fail-soft, NO re-send)**: a bounded wait (`FLEET_STARTUP_WAIT_TRIES × FLEET_STARTUP_WAIT_SLEEP`, default 5×3s) for evidence the child LEFT `idle` (ANY of: `agent get` status ≠ idle, revision moved past the pre-start baseline, session-file growth under `~/.pi/agent/sessions/<encoded-cwd>/`). If it never happens the launcher proceeds anyway — a truly empty/frozen pane is caught by the §7 done-wait liveness gate (`agent_alive`), exactly as before.
- **Resume (`--resume`, T-019)**: rebuilds the same CHILD_PROMPT + RESUME NOTICE and goes through the SAME native delivery — one code path, no second delivery mechanism.
Headless smoke: `tests/smoke-prompt-ack.sh` (mocked herdr/treehouse, isolated state): asserts ZERO `agent prompt` calls in the mock record, the `@<prompt-file>` in the `agent start` argv, the fail-soft residual wait on a nogrow child (→ §7 liveness fail path, rc=1, no re-send), and done via status-grow / session-file-grow children.
