...
What it does: creates a scratch repo in `/tmp/fleet-smoke-*` (git init + initial commit), isolates the state in `/tmp/fleet-smoke-state-*` via `FLEET_STATE_HOME` (the real fleet in `~/.pi/fleet` **is not touched**), launches `bin/herdr-launch.sh` with `--no-worktree` on a minimal child task and verifies: state json with `state == "done"` and non-empty `summary`, `esito.txt` containing `SMOKE_OK` in the scratch repo, done-marker consumed by the launcher, the transient child-prompt file (`<id>.child-prompt.md`) removed at exit while the durable brief (`tasks/<id>.brief.md`) is kept. The **headless native-launch contract** (no real herdr needed — mocked herdr/treehouse, isolated temp state) is covered by `tests/smoke-prompt-ack.sh`: exactly-one `@` argv element (absolute, existing, byte-identical to the complete CHILD_PROMPT), separate `--model provider/id` pair, spaces/metacharacter paths & briefs, zero `agent prompt`/readiness/ACK machinery, done+cleanup, start-failure and unreadable prompt-file → failed, 3 concurrent tasks without prompt cross-contamination, `--resume` prompt rebuild, and an installed-Pi `parseArgs` fixture — see `docs/ARCHITECTURE.md`, *native initial-request delivery*.
...
- Creates the child **tab in the fleet workspace** (`tab create --workspace <fleet> --cwd <worktree> --label <task-id> --no-focus`) → visible **only in the herdr agents sidebar** (never in the captain's tab or tab bar), then starts the **child pi with the brief as its native initial request**: the complete CHILD_PROMPT is materialized BEFORE the start to a private transient `$STATE_HOME/<id>.child-prompt.md` (umask 077, atomic tmp+mv, removed on every exit, per-task stale sweep for SIGKILL — durable briefs in `tasks/` are never touched) and passed as exactly ONE `@<prompt-file>` argv element after `agent start ... --`, separate from the `--model provider/id` pair (agent names 1–32 chars, unique per task; always the full `provider/id`, never a bare id)
- **Delivers the brief as the native initial request** (rules: cwd, detached HEAD → create a branch `fleet/<id>-<slug>` before committing, done-marker JSON, full markdown summary): pi reads the `@<file>` arg and prompts the session before its main loop — **zero `agent prompt`, no readiness/ACK machinery**; a frozen pane is caught by the liveness check below
